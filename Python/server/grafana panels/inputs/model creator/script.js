...
function toggleList(whichWay) {
	var ul = document.getElementById("custom-select-list");
	
	if (whichWay === 'Open') {

		// input_box.setAttribute("disabled", "");
		
		csList.classList.remove('hidden-all');
		csSelector.setAttribute('aria-expanded', 'true');
		var url = "http://127.0.0.1:5000/data/tags/"
		...
		.catch((error) => {
			...
			// Show tags on dropdown
			for(var i = 0; i < output.message.length; i++){
				// console.log(output.message[i])
				addDropdown(output.message[i], ul)
			}
		})
	} else {
		...
		// Delete previously generated tags
